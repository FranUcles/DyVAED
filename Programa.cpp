...
Dato combinar(int m, int inicio, int final, char A[], char B[], Dato parcial1, Dato parcial2) {
    ...
    for (int i = 1; i <= longitud; i++) {
        ...
        if (parcial3.getCoincidencias() == m)                           // Comprobamos si es el caso máximo, si lo es salimos
            return parcial3;
    }
...
Dato DyV(int m, int inicio, int final, char A[], char B[]) {
    ...
    int medio = inicio + elem/2 - 1;                                        // Dividimos las cadenas por la mitad
    Dato dato1 = DyV(m, inicio, medio, A, B);                               // Evaluamos la primera mitad
    if (dato1.getCoincidencias() == m)                                      // Si el máximo, salimos
        return dato1;
    Dato dato2 = DyV(m, medio + 1, final, A, B);                            // Evaluamos la segunda mitad
    if (dato2.getCoincidencias() == m)                                      // Si el máximo, salimos
        return dato2;
